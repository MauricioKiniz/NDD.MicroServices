...
{
    public delegate void DataReaderDelegate(IDataRecord reader);
    ...
    {
        private static readonly string VarBinaryConst = "varbinary";
        ...
        public SqlCommand CreateCommand(string qry, CommandType type, params object[] args)
        {
            ...
            for (int i = 0; i < args.Length; i++)
            {
                if (args[i] is string && i < (args.Length - 1))
                {
                    ...
                    if (paramName.StartsWith(VarBinaryConst, StringComparison.CurrentCultureIgnoreCase))
                    {
                        parm.SqlDbType = SqlDbType.VarBinary;
                        parm.DbType = DbType.Binary;
                        paramName = paramName.Remove(0, VarBinaryConst.Length);
                        parm.Size = -1;
                        parm.SqlValue = args[++i];
                    }
                    else
                    {
                        parm.Value = args[++i];
        ...
        public void ExecDataReader(string qry, DataReaderDelegate readerDelegate, params object[] args)
        {
            using (var dataReader = ExecDataReader(qry, args))
            {
                while (dataReader.Read())
                {
                    readerDelegate((IDataRecord)dataReader);
                }
            }
        }
